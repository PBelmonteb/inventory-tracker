...
import { ScanLine, PackageSearch, QrCode } from "lucide-react";

type Tab = "escanear" | "etiquetas";
...
export function EscanearView({
  ...
  // El operario solo tiene la pestaña de escaneo — generar/imprimir
  // etiquetas sigue siendo gestor-only, pero vivir aquí junto al escaneo
  // es más natural que enterrarlo solo en Administración (mismo QR de un
  // lado y del otro del ciclo).
  ...
  return (
    <div className={tab === "escanear" ? "mx-auto max-w-xl p-4 md:p-8" : "p-4 md:p-8"}>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold tracking-tight text-fg md:text-3xl">
          {tab === "escanear" ? "Escanear material" : "Etiquetas"}
        </h1>
        <p className="mt-1 text-sm text-muted">
          {tab === "escanear"
            ? "Lee el código del material para registrar un movimiento al instante."
            : "Genera e imprime los QR que luego se escanean aquí mismo."}
        </p>
      </div>
      ...
      {esGestor && (
        <div className="mb-6 flex gap-1 overflow-x-auto rounded-xl border border-line bg-surface p-1">
          ...
          >
            <QrCode className="h-4 w-4" /> Etiquetas
          </button>
        </div>
      )}
      ...
      {tab === "etiquetas" && esGestor && <EtiquetasView materiales={materiales} />}
    </div>
  );
}
